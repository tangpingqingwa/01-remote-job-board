...
import { getBoardListings, laneLabel, parseLane } from "../src/lib/board";
...
test("empty and closed-empty weeks stay honest — Claim #1, no invented role", () => {
  ...
  const closedCopy = closedEmpty.indexOf("Closed week history was empty");
  ...
  assert.match(closedEmpty, /Closed week history was empty/);
...
test("empty wall copy is rolling last 7 days from paid placement — not Period/Next reset Monday 00:00 UTC", () => {
  ...
  assert.match(occupied, /Later ranks in the rolling last 7 days/);
  ...
  assert.match(closed, /Closed week history 2026-W33 — read only/);
  assert.doesNotMatch(closed, /Period 2026-W33\. Next reset/);
  assert.doesNotMatch(closed, /Next reset 2026-08-24T00:00:00\.000Z/);
...
test("occupied live fact is rolling last 7 days from paid placement — not This week's remote wall", () => {
  ...
  assert.match(closed, /Closed week history 2026-W33 — read only/);
  assert.match(closed, /Week 2026-W33 is read-only week history/);
  assert.doesNotMatch(closed, /Period 2026-W33\. Next reset/);
  assert.doesNotMatch(
    closed,
...
test("occupied prize/later labels are rolling last 7 days from paid placement — not This week's #1", () => {
  const listings = rankListings(specTieRows);
  const occupied = renderToStaticMarkup(
    createElement(Board, {
      lane: "backend",
  ...
  const prizeLabelAt = occupied.indexOf('aria-label="Rolling last 7 days #1"');
  const laterLabelAt = occupied.indexOf(
    'aria-label="Later ranks in the rolling last 7 days"',
  );
  const applyAt = occupied.indexOf('data-first-click="apply"');
  const platesAt = occupied.indexOf("wall-plate");
  const listAt = occupied.indexOf('data-list-role="employer"');
  assert.ok(windowAt >= 0);
  assert.ok(factAt > windowAt);
  assert.ok(prizeLabelAt > factAt);
  assert.ok(laterLabelAt > prizeLabelAt);
  assert.ok(applyAt > prizeLabelAt && platesAt > applyAt && listAt > platesAt);
  ...
  assert.doesNotMatch(
    occupied,
    /data-rolling-prize|data-rolling-later|data-occupied-label-hop|data-prize-window-hop/,
  );
  ...
  assert.doesNotMatch(empty, /data-week-window/);
  assert.doesNotMatch(empty, /This week/);
  assert.doesNotMatch(empty, /aria-label="Rolling last 7 days #1"/);
  assert.doesNotMatch(empty, /Later ranks in the rolling last 7 days/);
  ...
  assert.match(closed, /aria-label="Closed week history #1"/);
  assert.match(closed, /aria-label="Later ranks in closed week history"/);
  assert.doesNotMatch(closed, /This week(?:'|&#x27;|&apos;)s #1/);
  assert.doesNotMatch(closed, /Later ranks this week/);
  assert.doesNotMatch(closed, /data-week-window="rolling-7d"/);
  assert.doesNotMatch(closed, /aria-label="Rolling last 7 days #1"/);
  assert.doesNotMatch(closed, /Later ranks in the rolling last 7 days/);
  assert.doesNotMatch(closed, /data-bid-form/);
  assert.doesNotMatch(closed, />Outbid</);
  assert.match(closed, /wall-rail/);
...
test("closed empty live-pointer is rolling last 7 days from paid placement — not this week's live wall", () => {
  ...
  assert.match(closedEmpty, /Closed week history 2026-W33 — read only/);
  assert.doesNotMatch(closedEmpty, /Period 2026-W33\. Next reset/);
  assert.match(closedEmpty, /Week 2026-W33 is read-only week history/);
  assert.doesNotMatch(
    closedEmpty,
    /This week(?:'|&#x27;|&apos;)s remote \(global\) Backend wall/,
  );
  ...
  assert.match(closedOccupied, /Closed week history 2026-W33 — read only/);
  assert.match(closedOccupied, /Week 2026-W33 is read-only week history/);
  assert.doesNotMatch(closedOccupied, /Period 2026-W33\. Next reset/);
  assert.doesNotMatch(closedOccupied, /Next reset 2026-08-24T00:00:00\.000Z/);
  assert.doesNotMatch(
    closedOccupied,
    /This week(?:'|&#x27;|&apos;)s remote \(global\) Backend wall/,
  );
  assert.match(closedOccupied, /aria-label="Closed week history #1"/);
  assert.match(closedOccupied, /aria-label="Later ranks in closed week history"/);
  assert.doesNotMatch(closedOccupied, /This week(?:'|&#x27;|&apos;)s #1/);
  assert.doesNotMatch(closedOccupied, /Later ranks this week/);
...
test("closed empty history fact is closed weekId — not this week's wall", () => {
  const listings = rankListings(specTieRows);
  const closedEmpty = renderToStaticMarkup(
    createElement(Board, {
      lane: "backend",
  ...
  const closedMark = closedEmpty.indexOf('data-empty-closed="true"');
  const factAt = closedEmpty.indexOf("Week 2026-W33 is read-only week history");
  const pointerAt = closedEmpty.indexOf('data-live-week=""');
  const pointerCopyAt = closedEmpty.indexOf(
    "Open the live Backend wall for the rolling last 7 days from paid placement",
  );
  assert.ok(
    factAt >= 0,
    "closed empty history fact must name the closed weekId as read-only week history",
  );
  assert.ok(closedMark >= 0 && factAt < closedMark);
  assert.ok(pointerAt > closedMark);
  assert.ok(pointerCopyAt > pointerAt);

  assert.match(closedEmpty, /data-period-live="false"/);
  assert.match(closedEmpty, /Closed week history 2026-W33 — read only/);
  assert.doesNotMatch(closedEmpty, /Period 2026-W33\. Next reset/);
  assert.doesNotMatch(closedEmpty, /Next reset 2026-08-24T00:00:00\.000Z/);
  assert.match(closedEmpty, /Week 2026-W33 is read-only week history/);
  assert.match(closedEmpty, /Rank is the bid/);
  assert.doesNotMatch(
    closedEmpty,
    /This week(?:'|&#x27;|&apos;)s remote \(global\) Backend wall/,
  );
  assert.doesNotMatch(closedEmpty, /this week(?:'|&#x27;|&apos;)s wall/);
  assert.match(closedEmpty, /data-empty-closed="true"/);
  assert.match(closedEmpty, /data-empty-honest=""/);
  assert.match(closedEmpty, /Bids are closed/);
  assert.match(closedEmpty, /Closed week history was empty/);
  ...
  assert.doesNotMatch(closedEmpty, /This remote \(global\) wall is empty/);
  assert.doesNotMatch(closedEmpty, /Not Monday 00:00 UTC/);
  assert.doesNotMatch(closedEmpty, /wall is the rolling last 7 days from paid placement/);
  assert.doesNotMatch(closedEmpty, /data-empty-window/);
  assert.doesNotMatch(closedEmpty, /data-bid-form/);
  assert.doesNotMatch(closedEmpty, />Outbid</);
  assert.doesNotMatch(closedEmpty, /Claim #1 for/);
  ...
  assert.doesNotMatch(
    closedEmpty,
    /data-closed-empty-fact|data-history-fact-hop|data-closed-weekid-hop|data-empty-history-hop/,
  );
  assert.doesNotMatch(closedEmpty, /data-list-after-apply-N|data-list-after-apply-eight/);
  assert.match(closedEmpty, /wall-rail/);
  ...
  assert.ok(occupied.indexOf("wall-plate") < occupied.indexOf('data-list-role="employer"'));
  assert.doesNotMatch(occupied, /class="wall-rail"/);
  assert.doesNotMatch(occupied, /Week 2026-W34 is read-only week history/);
  assert.doesNotMatch(occupied, /This week(?:'|&#x27;|&apos;)s #1/);
  assert.doesNotMatch(occupied, /Later ranks this week/);
  ...
  assert.match(empty, /The last 7 days from paid placement are empty/);
  assert.match(empty, /Claim #1 for/);
  assert.doesNotMatch(empty, /Week 2026-W34 is read-only week history/);
  assert.doesNotMatch(empty, /This week/);
  assert.doesNotMatch(empty, /data-week-window/);
  ...
  assert.match(unpaidHtml, /data-empty-window=""/);
  assert.doesNotMatch(unpaidHtml, /Ghost|Unpaid Staff Engineer/);
  assert.doesNotMatch(unpaidHtml, /Week 2026-W34 is read-only week history/);
  assert.doesNotMatch(unpaidHtml, /data-listing-card/);
});

test("closed occupied history fact is closed weekId — not this week's wall", () => {
  const listings = rankListings(specTieRows);
  const closedOccupied = renderToStaticMarkup(
    createElement(Board, {
      lane: "backend",
  ...
  const unpaidHtml = renderToStaticMarkup(
    createElement(Board, {
      ...
      listings: rankListings([
        fixtureListing({
          id: "lst_unpaid_closed_occupied_fact",
          company: "Ghost",
          title: "Unpaid Staff Engineer",
          bidUsd: 50_000,
          paidUsd: 0,
  ...
  const periodAt = closedOccupied.indexOf("Closed week history 2026-W33 — read only");
  const factAt = closedOccupied.indexOf("Week 2026-W33 is read-only week history");
  const prizeAt = closedOccupied.indexOf('data-prize-title=""');
  const applyAt = closedOccupied.indexOf(">Apply<");
  assert.ok(
    factAt >= 0,
    "closed occupied history fact must name the closed weekId as read-only week history",
  );
  assert.ok(periodAt >= 0 && factAt > periodAt);
  assert.ok(prizeAt > factAt && applyAt > prizeAt);

  assert.match(closedOccupied, /data-period-live="false"/);
  assert.match(closedOccupied, /Closed week history 2026-W33 — read only/);
  assert.doesNotMatch(closedOccupied, /Period 2026-W33\. Next reset/);
  assert.doesNotMatch(closedOccupied, /Next reset 2026-08-24T00:00:00\.000Z/);
  assert.match(closedOccupied, /Week 2026-W33 is read-only week history/);
  assert.match(closedOccupied, /Rank is the bid/);
  assert.doesNotMatch(
    closedOccupied,
    /This week(?:'|&#x27;|&apos;)s remote \(global\) Backend wall/,
  );
  assert.doesNotMatch(closedOccupied, /this week(?:'|&#x27;|&apos;)s wall/);
  assert.match(closedOccupied, /data-listing-card/);
  assert.match(closedOccupied, /data-prize-title=""/);
  assert.match(closedOccupied, /Staff Backend Engineer/);
  assert.match(closedOccupied, />Apply</);
  assert.match(closedOccupied, /\$21/);
  assert.match(closedOccupied, /9 clicks/);
  assert.match(closedOccupied, /aria-label="Closed week history #1"/);
  assert.match(closedOccupied, /aria-label="Later ranks in closed week history"/);
  assert.doesNotMatch(closedOccupied, /This week(?:'|&#x27;|&apos;)s #1/);
  assert.doesNotMatch(closedOccupied, /Later ranks this week/);
  assert.doesNotMatch(closedOccupied, /data-week-window="rolling-7d"/);
  assert.doesNotMatch(
    closedOccupied,
    /Rolling last 7 days from paid placement\. Week 2026-W33 is an audit label/,
  );
  assert.doesNotMatch(closedOccupied, /wall is the rolling last 7 days from paid placement/);
  assert.doesNotMatch(closedOccupied, /aria-label="Rolling last 7 days #1"/);
  assert.doesNotMatch(closedOccupied, /Later ranks in the rolling last 7 days/);
  assert.doesNotMatch(closedOccupied, /data-live-week/);
  assert.doesNotMatch(closedOccupied, /Open the live Backend wall/);
  assert.doesNotMatch(closedOccupied, /This remote \(global\) wall is empty/);
  assert.doesNotMatch(closedOccupied, /Not Monday 00:00 UTC/);
  assert.doesNotMatch(closedOccupied, /data-empty-window/);
  assert.doesNotMatch(closedOccupied, /data-bid-form/);
  assert.doesNotMatch(closedOccupied, />Outbid</);
  assert.doesNotMatch(closedOccupied, /Claim #1 for/);
  assert.doesNotMatch(closedOccupied, /data-first-click="apply"/);
  assert.doesNotMatch(closedOccupied, /data-first-click="claim"/);
  assert.doesNotMatch(
    closedOccupied,
    /data-closed-occupied-fact|data-occupied-history-hop|data-closed-occupied-weekid-hop|data-occupied-history-fact-hop/,
  );
  assert.doesNotMatch(closedOccupied, /data-list-after-apply-N|data-list-after-apply-eight/);
  assert.match(closedOccupied, /wall-rail/);
  assert.match(closedOccupied, /wall-plate/);
  assert.ok(closedOccupied.indexOf("wall-rail") < closedOccupied.indexOf("data-prize-title"));
...
test("closed occupied prize/later labels are closed week history — not this week's ranks", () => {
  const listings = rankListings(specTieRows);
  const closedOccupied = renderToStaticMarkup(
    createElement(Board, {
      lane: "backend",
  ...
  const unpaidHtml = renderToStaticMarkup(
    createElement(Board, {
      ...
      listings: rankListings([
        fixtureListing({
          id: "lst_unpaid_closed_occupied_labels",
          company: "Ghost",
          title: "Unpaid Staff Engineer",
          bidUsd: 50_000,
          paidUsd: 0,
  ...
  const factAt = closedOccupied.indexOf("Week 2026-W33 is read-only week history");
  const prizeLabelAt = closedOccupied.indexOf('aria-label="Closed week history #1"');
  const laterLabelAt = closedOccupied.indexOf(
    'aria-label="Later ranks in closed week history"',
  );
  const prizeAt = closedOccupied.indexOf('data-prize-title=""');
  const applyAt = closedOccupied.indexOf(">Apply<");
  assert.ok(
    prizeLabelAt >= 0,
    "closed occupied prize pack must name closed week history, not this week's #1",
  );
  assert.ok(
    laterLabelAt >= 0,
    "closed occupied later pack must name closed week history, not this week's ranks",
  );
  assert.ok(factAt >= 0 && prizeLabelAt > factAt);
  assert.ok(laterLabelAt > prizeLabelAt);
  assert.ok(prizeAt > prizeLabelAt && applyAt > prizeAt);
  ...
  assert.match(closedOccupied, /aria-label="Closed week history #1"/);
  assert.match(closedOccupied, /aria-label="Later ranks in closed week history"/);
  assert.match(closedOccupied, /data-prize-pack=""/);
  assert.match(closedOccupied, /data-later-pack=""/);
  assert.match(closedOccupied, /data-prize-title=""/);
  assert.match(closedOccupied, /Staff Backend Engineer/);
  assert.match(closedOccupied, /data-listing-card/);
  assert.match(closedOccupied, />Apply</);
  ...
  assert.doesNotMatch(closedOccupied, /This week(?:'|&#x27;|&apos;)s #1/);
  assert.doesNotMatch(closedOccupied, /Later ranks this week/);
  assert.doesNotMatch(closedOccupied, /this week(?:'|&#x27;|&apos;)s ranks/);
  assert.doesNotMatch(closedOccupied, /data-week-window="rolling-7d"/);
  assert.doesNotMatch(
    closedOccupied,
    /Rolling last 7 days from paid placement\. Week 2026-W33 is an audit label/,
  ...
  assert.doesNotMatch(
    closedOccupied,
    /data-closed-occupied-labels|data-history-label-hop|data-closed-prize-hop|data-occupied-history-label/,
  );
  assert.doesNotMatch(closedOccupied, /data-list-after-apply-N|data-list-after-apply-eight/);
  assert.match(closedOccupied, /wall-rail/);
  assert.match(closedOccupied, /wall-plate/);
  ...
  assert.doesNotMatch(occupied, /class="wall-rail"/);
  assert.doesNotMatch(occupied, /Week 2026-W34 is read-only week history/);
  assert.doesNotMatch(occupied, /Closed week history #1/);
  assert.doesNotMatch(occupied, /Later ranks in closed week history/);
  assert.doesNotMatch(occupied, /This week(?:'|&#x27;|&apos;)s #1/);
  assert.doesNotMatch(occupied, /Later ranks this week/);
  ...
  assert.match(empty, /Claim #1 for/);
  assert.doesNotMatch(empty, /Week 2026-W34 is read-only week history/);
  assert.doesNotMatch(empty, /Closed week history #1/);
  assert.doesNotMatch(empty, /Later ranks in closed week history/);
  assert.doesNotMatch(empty, /This week/);
  assert.doesNotMatch(empty, /data-week-window/);
  assert.doesNotMatch(empty, /aria-label="Rolling last 7 days #1"/);
  ...
  assert.doesNotMatch(closedEmpty, /Closed week history #1/);
  assert.doesNotMatch(closedEmpty, /Later ranks in closed week history/);
  assert.doesNotMatch(closedEmpty, /data-listing-card/);
  assert.doesNotMatch(closedEmpty, />Apply</);

  assert.match(unpaidHtml, /data-empty-window=""/);
  assert.doesNotMatch(unpaidHtml, /Ghost|Unpaid Staff Engineer/);
  assert.doesNotMatch(unpaidHtml, /Closed week history #1/);
  assert.doesNotMatch(unpaidHtml, /Later ranks in closed week history/);
  assert.doesNotMatch(unpaidHtml, /data-listing-card/);
});

test("closed occupied period stamp is closed week history — not a live Next reset clock", () => {
  const listings = rankListings(specTieRows);
  const closedOccupied = renderToStaticMarkup(
    createElement(Board, {
      lane: "backend",
  ...
  const unpaidHtml = renderToStaticMarkup(
    createElement(Board, {
      ...
      listings: rankListings([
        fixtureListing({
          id: "lst_unpaid_closed_occupied_period",
          company: "Ghost",
          title: "Unpaid Staff Engineer",
          bidUsd: 50_000,
          paidUsd: 0,
  ...
  assert.ok(
    periodAt >= 0,
    "closed occupied period stamp must name closed week history, not a live Next reset clock",
  );
  assert.ok(factAt > periodAt);
  assert.ok(prizeLabelAt > factAt && laterLabelAt > prizeLabelAt);
  assert.ok(prizeAt > prizeLabelAt && applyAt > prizeAt);
  ...
  assert.doesNotMatch(closedOccupied, /Period 2026-W33\. Next reset/);
  assert.doesNotMatch(closedOccupied, /Next reset 2026-08-24T00:00:00\.000Z/);
  assert.doesNotMatch(closedOccupied, /Next reset/);
  assert.match(closedOccupied, /Week 2026-W33 is read-only week history/);
  assert.match(closedOccupied, /Rank is the bid/);
  assert.match(closedOccupied, /aria-label="Closed week history #1"/);
  assert.match(closedOccupied, /aria-label="Later ranks in closed week history"/);
  ...
  assert.doesNotMatch(
    closedOccupied,
    /data-closed-occupied-period|data-history-period-hop|data-closed-period-hop|data-occupied-period-stamp/,
  );
  assert.doesNotMatch(closedOccupied, /data-list-after-apply-N|data-list-after-apply-eight/);
  assert.match(closedOccupied, /wall-rail/);
  assert.match(closedOccupied, /wall-plate/);
  ...
  assert.match(
    occupied,
    /Rolling last 7 days from paid placement\. Week 2026-W34 is an audit label\. Next reset/,
  );
  assert.match(
    occupied,
    /This remote \(global\) Backend wall is the rolling last 7 days from paid placement/,
  ...
  assert.ok(occupied.indexOf("wall-plate") < occupied.indexOf('data-list-role="employer"'));
  assert.doesNotMatch(occupied, /class="wall-rail"/);
  assert.doesNotMatch(occupied, /Closed week history 2026-W34 — read only/);
  assert.doesNotMatch(occupied, /Week 2026-W34 is read-only week history/);
  assert.doesNotMatch(occupied, /Closed week history #1/);
  assert.doesNotMatch(occupied, /Later ranks in closed week history/);
  assert.doesNotMatch(occupied, /This week(?:'|&#x27;|&apos;)s #1/);
  ...
  assert.match(empty, /The last 7 days from paid placement are empty/);
  assert.match(empty, /Claim #1 for/);
  assert.doesNotMatch(empty, /Closed week history 2026-W34 — read only/);
  assert.doesNotMatch(empty, /Week 2026-W34 is read-only week history/);
  assert.doesNotMatch(empty, /Closed week history #1/);
  assert.doesNotMatch(empty, /Later ranks in closed week history/);
  assert.doesNotMatch(empty, /Period 2026-W34\. Next reset/);
  ...
  assert.doesNotMatch(closedEmpty, /Next reset 2026-08-24T00:00:00\.000Z/);
  assert.match(closedEmpty, /Week 2026-W33 is read-only week history/);
  assert.doesNotMatch(closedEmpty, /Next reset/);
  assert.doesNotMatch(
    closedEmpty,
    /This week(?:'|&#x27;|&apos;)s remote \(global\) Backend wall/,
  );
  ...
  assert.match(unpaidHtml, /data-empty-window=""/);
  assert.doesNotMatch(unpaidHtml, /Ghost|Unpaid Staff Engineer/);
  assert.doesNotMatch(unpaidHtml, /Closed week history/);
  assert.doesNotMatch(unpaidHtml, /data-listing-card/);
});

test("closed empty period stamp is closed week history — not a live Next reset clock", () => {
  const listings = rankListings(specTieRows);
  const closedEmpty = renderToStaticMarkup(
    createElement(Board, {
      lane: "backend",
  ...
  const unpaidHtml = renderToStaticMarkup(
    createElement(Board, {
      ...
      listings: rankListings([
        fixtureListing({
          id: "lst_unpaid_closed_empty_period",
          company: "Ghost",
          title: "Unpaid Staff Engineer",
          bidUsd: 50_000,
          paidUsd: 0,
  ...
  const periodAt = closedEmpty.indexOf("Closed week history 2026-W33 — read only");
  const factAt = closedEmpty.indexOf("Week 2026-W33 is read-only week history");
  const closedMark = closedEmpty.indexOf('data-empty-closed="true"');
  const pointerAt = closedEmpty.indexOf('data-live-week=""');
  const pointerCopyAt = closedEmpty.indexOf(
  ...
  assert.ok(
    periodAt >= 0,
    "closed empty period stamp must name closed week history, not a live Next reset clock",
  );
  assert.ok(factAt > periodAt);
  assert.ok(closedMark > factAt && pointerAt > closedMark);
  assert.ok(pointerCopyAt > pointerAt);
  ...
  assert.doesNotMatch(
    closedEmpty,
    /data-closed-empty-period|data-empty-period-hop|data-closed-empty-period-hop|data-empty-period-stamp/,
  );
  assert.doesNotMatch(closedEmpty, /data-list-after-apply-N|data-list-after-apply-eight/);
  assert.match(closedEmpty, /wall-rail/);
...
test("closed empty kicker is closed week history — not a generic Closed week", () => {
  const listings = rankListings(specTieRows);
  const closedEmpty = renderToStaticMarkup(
    createElement(Board, {
      lane: "backend",
  ...
  const unpaidHtml = renderToStaticMarkup(
    createElement(Board, {
      ...
      listings: rankListings([
        fixtureListing({
          id: "lst_unpaid_closed_empty_kicker",
          company: "Ghost",
          title: "Unpaid Staff Engineer",
          bidUsd: 50_000,
          paidUsd: 0,
  ...
  const periodAt = closedEmpty.indexOf("Closed week history 2026-W33 — read only");
  const factAt = closedEmpty.indexOf("Week 2026-W33 is read-only week history");
  const kickerAt = closedEmpty.indexOf('class="empty-lane-kicker">Closed week history');
  const genericKickerAt = closedEmpty.indexOf('class="empty-lane-kicker">Closed week<');
  const closedMark = closedEmpty.indexOf('data-empty-closed="true"');
  const pointerAt = closedEmpty.indexOf('data-live-week=""');
  const pointerCopyAt = closedEmpty.indexOf(
    "Open the live Backend wall for the rolling last 7 days from paid placement",
  ...
  assert.ok(
    kickerAt >= 0,
    "closed empty kicker must name closed week history, not a generic Closed week",
  );
  assert.equal(genericKickerAt, -1);
  assert.ok(periodAt >= 0 && factAt > periodAt);
  assert.ok(kickerAt > factAt && closedMark >= 0 && pointerAt > kickerAt);
  assert.ok(pointerCopyAt > pointerAt);

  assert.match(closedEmpty, /data-period-live="false"/);
  assert.match(closedEmpty, /Closed week history 2026-W33 — read only/);
  assert.match(closedEmpty, /class="empty-lane-kicker">Closed week history</);
  assert.doesNotMatch(closedEmpty, /class="empty-lane-kicker">Closed week</);
  assert.doesNotMatch(closedEmpty, /Period 2026-W33\. Next reset/);
  assert.doesNotMatch(closedEmpty, /Next reset 2026-08-24T00:00:00\.000Z/);
  assert.doesNotMatch(closedEmpty, /Next reset/);
  assert.match(closedEmpty, /Week 2026-W33 is read-only week history/);
  ...
  assert.doesNotMatch(closedEmpty, /Empty bay/);
  assert.doesNotMatch(closedEmpty, /Closed week history #1/);
  assert.doesNotMatch(closedEmpty, /Later ranks in closed week history/);
  assert.doesNotMatch(closedEmpty, /data-week-window="rolling-7d"/);
  assert.doesNotMatch(
  ...
  assert.doesNotMatch(
    closedEmpty,
    /data-closed-empty-kicker|data-empty-kicker-hop|data-closed-kicker-hop|data-empty-history-kicker/,
  );
  assert.doesNotMatch(closedEmpty, /data-list-after-apply-N|data-list-after-apply-eight/);
  assert.match(closedEmpty, /wall-rail/);
  ...
  assert.ok(occupied.indexOf("wall-plate") < occupied.indexOf('data-list-role="employer"'));
  assert.doesNotMatch(occupied, /class="wall-rail"/);
  assert.doesNotMatch(occupied, /empty-lane-kicker/);
  assert.doesNotMatch(occupied, /Closed week history 2026-W34 — read only/);
  assert.doesNotMatch(occupied, /Week 2026-W34 is read-only week history/);
  assert.doesNotMatch(occupied, /Closed week history #1/);
  assert.doesNotMatch(occupied, /Later ranks in closed week history/);
  ...
  assert.doesNotMatch(empty, /Closed week history 2026-W34 — read only/);
  assert.doesNotMatch(empty, /Week 2026-W34 is read-only week history/);
  assert.doesNotMatch(empty, /class="empty-lane-kicker">Closed week history</);
  assert.doesNotMatch(empty, /Closed week history #1/);
  assert.doesNotMatch(empty, /Later ranks in closed week history/);
  assert.doesNotMatch(empty, /Period 2026-W34\. Next reset/);
  assert.doesNotMatch(empty, /Next reset/);
  ...
  assert.match(closedOccupied, /aria-label="Closed week history #1"/);
  assert.match(closedOccupied, /aria-label="Later ranks in closed week history"/);
  assert.doesNotMatch(closedOccupied, /empty-lane-kicker/);
  assert.match(closedOccupied, /data-listing-card/);
  assert.match(closedOccupied, />Apply</);
  assert.doesNotMatch(closedOccupied, /This week(?:'|&#x27;|&apos;)s #1/);
  assert.doesNotMatch(closedOccupied, /Later ranks this week/);
  ...
  assert.doesNotMatch(unpaidHtml, /Ghost|Unpaid Staff Engineer/);
  assert.doesNotMatch(unpaidHtml, /Closed week history/);
  assert.doesNotMatch(unpaidHtml, /empty-lane-kicker/);
  assert.doesNotMatch(unpaidHtml, /data-listing-card/);
});

test("closed empty body is closed week history — not a generic empty lane", () => {
  const listings = rankListings(specTieRows);
  const closedEmpty = renderToStaticMarkup(
    createElement(Board, {
      lane: "backend",
  ...
  const unpaidHtml = renderToStaticMarkup(
    createElement(Board, {
      ...
      listings: rankListings([
        fixtureListing({
          id: "lst_unpaid_closed_empty_body",
          company: "Ghost",
          title: "Unpaid Staff Engineer",
          bidUsd: 50_000,
          paidUsd: 0,
  ...
  const factAt = closedEmpty.indexOf("Week 2026-W33 is read-only week history");
  const kickerAt = closedEmpty.indexOf('class="empty-lane-kicker">Closed week history');
  const bodyAt = closedEmpty.indexOf("No listings in closed week history");
  const wasEmptyAt = closedEmpty.indexOf("Closed week history was empty");
  const genericLaneAt = closedEmpty.indexOf("This lane was empty");
  const genericPeriodAt = closedEmpty.indexOf("No listings this period");
  const closedMark = closedEmpty.indexOf('data-empty-closed="true"');
  const pointerAt = closedEmpty.indexOf('data-live-week=""');
  const pointerCopyAt = closedEmpty.indexOf(
    "Open the live Backend wall for the rolling last 7 days from paid placement",
  ...
  assert.ok(
    bodyAt >= 0 && wasEmptyAt >= 0,
    "closed empty body must name closed week history, not a generic empty lane",
  );
  assert.equal(genericLaneAt, -1);
  assert.equal(genericPeriodAt, -1);
  assert.ok(periodAt >= 0 && factAt > periodAt);
  assert.ok(kickerAt > factAt && closedMark >= 0);
  assert.ok(bodyAt > kickerAt && wasEmptyAt > bodyAt && pointerAt > wasEmptyAt);
  assert.ok(pointerCopyAt > pointerAt);

  assert.match(closedEmpty, /data-period-live="false"/);
  assert.match(closedEmpty, /Closed week history 2026-W33 — read only/);
  assert.match(closedEmpty, /class="empty-lane-kicker">Closed week history</);
  assert.match(closedEmpty, /No listings in closed week history in Backend/);
  assert.match(closedEmpty, /Closed week history was empty/);
  assert.doesNotMatch(closedEmpty, /This lane was empty/);
  assert.doesNotMatch(closedEmpty, /No listings this period/);
  assert.doesNotMatch(closedEmpty, /This lane is empty/);
  assert.doesNotMatch(closedEmpty, /Period 2026-W33\. Next reset/);
  assert.doesNotMatch(closedEmpty, /Next reset 2026-08-24T00:00:00\.000Z/);
  assert.doesNotMatch(closedEmpty, /Next reset/);
  assert.match(closedEmpty, /Week 2026-W33 is read-only week history/);
  ...
  assert.doesNotMatch(
    closedEmpty,
    /data-closed-empty-body|data-empty-body-hop|data-closed-body-hop|data-empty-history-body/,
  );
  assert.doesNotMatch(closedEmpty, /data-list-after-apply-N|data-list-after-apply-eight/);
  assert.match(closedEmpty, /wall-rail/);
  ...
  assert.doesNotMatch(occupied, /Closed week history 2026-W34 — read only/);
  assert.doesNotMatch(occupied, /Week 2026-W34 is read-only week history/);
  assert.doesNotMatch(occupied, /No listings in closed week history/);
  assert.doesNotMatch(occupied, /Closed week history was empty/);
  assert.doesNotMatch(occupied, /Closed week history #1/);
  assert.doesNotMatch(occupied, /Later ranks in closed week history/);
  assert.doesNotMatch(occupied, /This week(?:'|&#x27;|&apos;)s #1/);
  assert.doesNotMatch(occupied, /Later ranks this week/);
  ...
  assert.match(empty, /Not Monday 00:00 UTC/);
  assert.match(empty, /The last 7 days from paid placement are empty/);
  assert.match(empty, /No listings this period/);
  assert.match(empty, /This lane is empty/);
  assert.match(empty, /Claim #1 for/);
  assert.doesNotMatch(empty, /Closed week history 2026-W34 — read only/);
  assert.doesNotMatch(empty, /Week 2026-W34 is read-only week history/);
  assert.doesNotMatch(empty, /class="empty-lane-kicker">Closed week history</);
  assert.doesNotMatch(empty, /No listings in closed week history/);
  assert.doesNotMatch(empty, /Closed week history was empty/);
  assert.doesNotMatch(empty, /This lane was empty/);
  assert.doesNotMatch(empty, /Closed week history #1/);
  assert.doesNotMatch(empty, /Later ranks in closed week history/);
  assert.doesNotMatch(empty, /Period 2026-W34\. Next reset/);
  assert.doesNotMatch(empty, /Next reset/);
  ...
  assert.match(closedOccupied, /aria-label="Later ranks in closed week history"/);
  assert.doesNotMatch(closedOccupied, /empty-lane-kicker/);
  assert.doesNotMatch(closedOccupied, /No listings in closed week history/);
  assert.doesNotMatch(closedOccupied, /Closed week history was empty/);
  assert.doesNotMatch(closedOccupied, /This lane was empty/);
  assert.doesNotMatch(closedOccupied, /No listings this period/);
  assert.match(closedOccupied, /data-listing-card/);
  assert.match(closedOccupied, />Apply</);
  assert.doesNotMatch(closedOccupied, /This week(?:'|&#x27;|&apos;)s #1/);
  assert.doesNotMatch(closedOccupied, /Later ranks this week/);
...
test("closed empty bids-closed line is closed week history — not a generic closed lane", () => {
  const listings = rankListings(specTieRows);
  const closedEmpty = renderToStaticMarkup(
    createElement(Board, {
      lane: "backend",
  ...
  const unpaidHtml = renderToStaticMarkup(
    createElement(Board, {
      ...
      listings: rankListings([
        fixtureListing({
          id: "lst_unpaid_closed_empty_bids_closed",
          company: "Ghost",
          title: "Unpaid Staff Engineer",
          bidUsd: 50_000,
          paidUsd: 0,
  ...
  const bodyAt = closedEmpty.indexOf("No listings in closed week history");
  const wasEmptyAt = closedEmpty.indexOf("Closed week history was empty");
  const bidsClosedAt = closedEmpty.indexOf("Bids are closed in closed week history");
  const genericBidsClosedAt = closedEmpty.indexOf("Bids are closed.");
  const closedMark = closedEmpty.indexOf('data-empty-closed="true"');
  const pointerAt = closedEmpty.indexOf('data-live-week=""');
  const pointerCopyAt = closedEmpty.indexOf(
    "Open the live Backend wall for the rolling last 7 days from paid placement",
  ...
  assert.ok(
    bidsClosedAt >= 0,
    "closed empty bids-closed line must name closed week history, not a generic closed lane",
  );
  assert.equal(genericBidsClosedAt, -1);
  assert.ok(periodAt >= 0 && factAt > periodAt);
  assert.ok(kickerAt > factAt && closedMark >= 0);
  assert.ok(bodyAt > kickerAt && wasEmptyAt > bodyAt && bidsClosedAt > wasEmptyAt);
  assert.ok(pointerAt > bidsClosedAt && pointerCopyAt > pointerAt);
  ...
  assert.match(closedEmpty, /No listings in closed week history in Backend/);
  assert.match(closedEmpty, /Closed week history was empty/);
  assert.match(closedEmpty, /Bids are closed in closed week history/);
  assert.doesNotMatch(closedEmpty, /Bids are closed\./);
  assert.doesNotMatch(closedEmpty, /This lane was empty/);
  assert.doesNotMatch(closedEmpty, /No listings this period/);
  assert.doesNotMatch(closedEmpty, /This lane is empty/);
  assert.doesNotMatch(closedEmpty, /Period 2026-W33\. Next reset/);
  ...
  assert.doesNotMatch(
    closedEmpty,
    /data-closed-empty-bids|data-bids-closed-hop|data-closed-bids-hop|data-empty-history-bids/,
  );
  assert.doesNotMatch(closedEmpty, /data-list-after-apply-N|data-list-after-apply-eight/);
  assert.match(closedEmpty, /wall-rail/);
  ...
  assert.doesNotMatch(occupied, /class="wall-rail"/);
  assert.doesNotMatch(occupied, /empty-lane-kicker/);
  assert.doesNotMatch(occupied, /Bids are closed in closed week history/);
  assert.doesNotMatch(occupied, /Closed week history 2026-W34 — read only/);
  assert.doesNotMatch(occupied, /Week 2026-W34 is read-only week history/);
  assert.doesNotMatch(occupied, /No listings in closed week history/);
  assert.doesNotMatch(occupied, /Closed week history was empty/);
  ...
  assert.match(empty, /This lane is empty/);
  assert.match(empty, /Claim #1 for/);
  assert.doesNotMatch(empty, /Bids are closed in closed week history/);
  assert.doesNotMatch(empty, /Closed week history 2026-W34 — read only/);
  assert.doesNotMatch(empty, /Week 2026-W34 is read-only week history/);
  assert.doesNotMatch(empty, /class="empty-lane-kicker">Closed week history</);
  assert.doesNotMatch(empty, /No listings in closed week history/);
  ...
  assert.match(closedOccupied, /aria-label="Later ranks in closed week history"/);
  assert.doesNotMatch(closedOccupied, /empty-lane-kicker/);
  assert.doesNotMatch(closedOccupied, /Bids are closed in closed week history/);
  assert.doesNotMatch(closedOccupied, /No listings in closed week history/);
  assert.doesNotMatch(closedOccupied, /Closed week history was empty/);
  assert.doesNotMatch(closedOccupied, /This lane was empty/);
  assert.doesNotMatch(closedOccupied, /No listings this period/);
...
test("closed empty function-rail is closed week history — not generic Function lanes", () => {
  const listings = rankListings(specTieRows);
  const closedEmpty = renderToStaticMarkup(
    createElement(Board, {
      lane: "backend",
  ...
  const unpaidHtml = renderToStaticMarkup(
    createElement(Board, {
      ...
      listings: rankListings([
        fixtureListing({
          id: "lst_unpaid_closed_empty_rail",
          company: "Ghost",
          title: "Unpaid Staff Engineer",
          bidUsd: 50_000,
          paidUsd: 0,
  ...
  const railAt = closedEmpty.indexOf('class="wall-rail"');
  const railKickerAt = closedEmpty.indexOf(
    'class="wall-rail-kicker">Closed week history',
  );
  const genericRailKickerAt = closedEmpty.indexOf(
    'class="wall-rail-kicker">Function lanes',
  );
  const railAriaAt = closedEmpty.indexOf('aria-label="Closed week history"');
  const genericRailAriaAt = closedEmpty.indexOf('aria-label="Function lanes"');
  const periodAt = closedEmpty.indexOf("Closed week history 2026-W33 — read only");
  const factAt = closedEmpty.indexOf("Week 2026-W33 is read-only week history");
  const kickerAt = closedEmpty.indexOf('class="empty-lane-kicker">Closed week history');
  const bodyAt = closedEmpty.indexOf("No listings in closed week history");
  ...
  assert.ok(
    railKickerAt >= 0,
    "closed empty function-rail must name closed week history, not generic Function lanes",
  );
  assert.equal(genericRailKickerAt, -1);
  assert.equal(genericRailAriaAt, -1);
  assert.ok(railAt >= 0 && railKickerAt > railAt);
  assert.ok(railAriaAt > railKickerAt);
  assert.ok(periodAt > railAriaAt && factAt > periodAt);
  assert.ok(kickerAt > factAt && closedMark >= 0);
  assert.ok(bodyAt > kickerAt && wasEmptyAt > bodyAt && bidsClosedAt > wasEmptyAt);
  assert.ok(pointerAt > bidsClosedAt && pointerCopyAt > pointerAt);

  assert.match(closedEmpty, /data-period-live="false"/);
  assert.match(closedEmpty, /class="wall-rail"/);
  assert.match(closedEmpty, /class="wall-rail-kicker">Closed week history</);
  assert.match(closedEmpty, /aria-label="Closed week history"/);
  assert.doesNotMatch(closedEmpty, /class="wall-rail-kicker">Function lanes</);
  assert.doesNotMatch(closedEmpty, /aria-label="Function lanes"/);
  assert.doesNotMatch(closedEmpty, /Function lanes/);
  assert.match(closedEmpty, /Closed week history 2026-W33 — read only/);
  assert.match(closedEmpty, /class="empty-lane-kicker">Closed week history</);
  assert.match(closedEmpty, /No listings in closed week history in Backend/);
  assert.match(closedEmpty, /Closed week history was empty/);
  ...
  assert.doesNotMatch(
    closedEmpty,
    /data-closed-empty-rail|data-function-rail-hop|data-closed-rail-hop|data-empty-history-rail/,
  );
  assert.doesNotMatch(closedEmpty, /data-list-after-apply-N|data-list-after-apply-eight/);
  assert.match(closedEmpty, /wall-plate/);
  ...
  assert.match(occupied, /data-first-click="apply"/);
  assert.match(occupied, />Apply</);
  assert.match(occupied, /class="wall-rail-kicker">Function lanes</);
  assert.match(occupied, /aria-label="Function lanes"/);
  assert.ok(occupied.indexOf('data-first-click="apply"') < occupied.indexOf("wall-plate"));
  assert.ok(occupied.indexOf("wall-plate") < occupied.indexOf('data-list-role="employer"'));
  assert.ok(
    occupied.indexOf('data-first-click="apply"') <
      occupied.indexOf('class="wall-rail-kicker">Function lanes'),
  );
  assert.doesNotMatch(occupied, /class="wall-rail"/);
  assert.doesNotMatch(occupied, /empty-lane-kicker/);
  assert.doesNotMatch(occupied, /class="wall-rail-kicker">Closed week history</);
  assert.doesNotMatch(occupied, /aria-label="Closed week history"/);
  assert.doesNotMatch(occupied, /Bids are closed in closed week history/);
  assert.doesNotMatch(occupied, /Closed week history 2026-W34 — read only/);
  assert.doesNotMatch(occupied, /Week 2026-W34 is read-only week history/);
  assert.doesNotMatch(occupied, /No listings in closed week history/);
  ...
  assert.match(empty, /This lane is empty/);
  assert.match(empty, /Claim #1 for/);
  assert.match(empty, /Function lanes/);
  assert.match(empty, /aria-label="Function lanes"/);
  assert.doesNotMatch(empty, /class="wall-rail"/);
  assert.doesNotMatch(empty, /class="wall-rail-kicker"/);
  assert.doesNotMatch(empty, /aria-label="Closed week history"/);
  assert.doesNotMatch(empty, /Bids are closed in closed week history/);
  assert.doesNotMatch(empty, /Closed week history 2026-W34 — read only/);
  assert.doesNotMatch(empty, /Week 2026-W34 is read-only week history/);
  assert.doesNotMatch(empty, /class="empty-lane-kicker">Closed week history</);
  ...
  assert.match(closedOccupied, /aria-label="Closed week history #1"/);
  assert.match(closedOccupied, /aria-label="Later ranks in closed week history"/);
  assert.match(closedOccupied, /class="wall-rail-kicker">Closed week history</);
  assert.match(closedOccupied, /aria-label="Closed week history"/);
  assert.doesNotMatch(closedOccupied, /class="wall-rail-kicker">Function lanes</);
  assert.doesNotMatch(closedOccupied, /aria-label="Function lanes"/);
  assert.doesNotMatch(closedOccupied, /Function lanes/);
  assert.doesNotMatch(closedOccupied, /empty-lane-kicker/);
  assert.doesNotMatch(closedOccupied, /Bids are closed in closed week history/);
  assert.doesNotMatch(closedOccupied, /No listings in closed week history/);
  assert.doesNotMatch(closedOccupied, /Closed week history was empty/);
  ...
  assert.ok(closedOccupied.indexOf("wall-rail") < closedOccupied.indexOf("data-prize-title"));
  assert.ok(
    closedOccupied.indexOf('class="wall-rail-kicker">Closed week history') <
      closedOccupied.indexOf("data-prize-title"),
  );
  ...
  assert.doesNotMatch(unpaidHtml, /Closed week history/);
  assert.doesNotMatch(unpaidHtml, /empty-lane-kicker/);
  assert.doesNotMatch(unpaidHtml, /class="wall-rail-kicker">Closed week history</);
  assert.doesNotMatch(unpaidHtml, /data-listing-card/);
});

test("closed occupied function-rail is closed week history — not generic Function lanes", () => {
  const listings = rankListings(specTieRows);
  const closedOccupied = renderToStaticMarkup(
    createElement(Board, {
      lane: "backend",
  ...
  const unpaidHtml = renderToStaticMarkup(
    createElement(Board, {
      ...
      listings: rankListings([
        fixtureListing({
          id: "lst_unpaid_closed_occupied_rail",
          company: "Ghost",
          title: "Unpaid Staff Engineer",
          bidUsd: 50_000,
          paidUsd: 0,
  ...
  const railAt = closedOccupied.indexOf('class="wall-rail"');
  const railKickerAt = closedOccupied.indexOf(
    'class="wall-rail-kicker">Closed week history',
  );
  const genericRailKickerAt = closedOccupied.indexOf(
    'class="wall-rail-kicker">Function lanes',
  );
  const railAriaAt = closedOccupied.indexOf('aria-label="Closed week history"');
  const genericRailAriaAt = closedOccupied.indexOf('aria-label="Function lanes"');
  const periodAt = closedOccupied.indexOf("Closed week history 2026-W33 — read only");
  const factAt = closedOccupied.indexOf("Week 2026-W33 is read-only week history");
  const prizeLabelAt = closedOccupied.indexOf('aria-label="Closed week history #1"');
  const laterLabelAt = closedOccupied.indexOf(
  ...
  assert.match(closedOccupied, /data-period-live="false"/);
  assert.match(closedOccupied, /class="wall-rail"/);
  assert.match(closedOccupied, /class="wall-rail-kicker">Closed week history</);
  assert.match(closedOccupied, /aria-label="Closed week history"/);
  assert.doesNotMatch(closedOccupied, /class="wall-rail-kicker">Function lanes</);
  assert.doesNotMatch(closedOccupied, /aria-label="Function lanes"/);
  ...
  assert.doesNotMatch(
    closedOccupied,
    /data-closed-occupied-rail|data-occupied-function-rail-hop|data-closed-occupied-rail-hop|data-occupied-history-rail/,
  );
  assert.doesNotMatch(closedOccupied, /data-list-after-apply-N|data-list-after-apply-eight/);
  assert.match(closedOccupied, /wall-plate/);
  assert.ok(closedOccupied.indexOf("wall-rail") < closedOccupied.indexOf("data-prize-title"));
...
test("closed occupied function plates are closed week history — not live Function lanes", () => {
  const listings = rankListings(specTieRows);
  const closedOccupied = renderToStaticMarkup(
    createElement(Board, {
      lane: "backend",
  ...
  const unpaidHtml = renderToStaticMarkup(
    createElement(Board, {
      ...
      listings: rankListings([
        fixtureListing({
          id: "lst_unpaid_closed_occupied_plates",
          company: "Ghost",
          title: "Unpaid Staff Engineer",
          bidUsd: 50_000,
          paidUsd: 0,
  ...
  const firstHistoryPlateAt = closedOccupied.indexOf(">Backend week history<");
  const genericPlateAt = closedOccupied.search(
    /data-lane="backend"[^>]*>Backend</,
  );
  const periodAt = closedOccupied.indexOf("Closed week history 2026-W33 — read only");
  const factAt = closedOccupied.indexOf("Week 2026-W33 is read-only week history");
  const prizeAt = closedOccupied.indexOf('data-prize-title=""');
  const applyAt = closedOccupied.indexOf(">Apply<");
  assert.ok(
    firstHistoryPlateAt >= 0,
    "closed occupied function plates must name closed week history, not live Function lanes",
  );
  assert.equal(genericPlateAt, -1);
  assert.ok(railKickerAt >= 0 && firstHistoryPlateAt > railKickerAt);
  assert.ok(periodAt < prizeAt && factAt < prizeAt && applyAt > prizeAt);

  for (const item of FUNCTION_LANES) {
    const name = laneLabel(item);
    assert.match(
      closedOccupied,
      new RegExp(`data-lane="${item}"[^>]*>${name} week history<`),
    );
    assert.doesNotMatch(
      closedOccupied,
      new RegExp(`data-lane="${item}"[^>]*>${name}<`),
    );
  }
  assert.equal((closedOccupied.match(/week history<\/a>/g) ?? []).length, 8);
  assert.equal((closedOccupied.match(/class="wall-plate"/g) ?? []).length, 8);
  ...
  assert.doesNotMatch(
    closedOccupied,
    /data-closed-occupied-plates|data-occupied-function-plates-hop|data-closed-occupied-plates-hop|data-occupied-history-plates/,
  );
  assert.doesNotMatch(closedOccupied, /data-list-after-apply-N|data-list-after-apply-eight/);
  assert.match(closedOccupied, /wall-plate/);
  assert.ok(closedOccupied.indexOf("wall-rail") < closedOccupied.indexOf("data-prize-title"));
  assert.ok(
    closedOccupied.indexOf(">Backend week history<") <
      closedOccupied.indexOf("data-prize-title"),
  );
  ...
  assert.equal((occupied.match(/week history<\/a>/g) ?? []).length, 0);
  assert.ok(occupied.indexOf('data-first-click="apply"') < occupied.indexOf("wall-plate"));
  assert.ok(occupied.indexOf("wall-plate") < occupied.indexOf('data-list-role="employer"'));
  assert.ok(
    occupied.indexOf('data-first-click="apply"') <
  ...
  assert.doesNotMatch(empty, /data-week-window/);
  assert.doesNotMatch(empty, /aria-label="Rolling last 7 days #1"/);
  assert.doesNotMatch(empty, /data-lane="backend"[^>]*>Backend week history</);
  assert.equal((empty.match(/week history<\/a>/g) ?? []).length, 0);
  ...
  assert.doesNotMatch(closedEmpty, /aria-label="Function lanes"/);
  assert.doesNotMatch(closedEmpty, /Function lanes/);
  assert.match(closedEmpty, /data-lane="backend"[^>]*>Backend week history</);
  assert.doesNotMatch(closedEmpty, /data-lane="backend"[^>]*>Backend</);
  assert.equal((closedEmpty.match(/week history<\/a>/g) ?? []).length, 8);
  assert.match(closedEmpty, /No listings in closed week history in Backend/);
  assert.match(closedEmpty, /Bids are closed in closed week history/);
  assert.doesNotMatch(closedEmpty, /data-listing-card/);
  assert.doesNotMatch(closedEmpty, />Apply</);
  ...
  assert.doesNotMatch(unpaidHtml, /class="wall-rail-kicker">Closed week history</);
  assert.doesNotMatch(unpaidHtml, /data-listing-card/);
  assert.doesNotMatch(unpaidHtml, /Backend week history/);
});

test("closed empty function plates are closed week history — not live Function lanes", () => {
  const listings = rankListings(specTieRows);
  const closedEmpty = renderToStaticMarkup(
    createElement(Board, {
      lane: "backend",
  ...
  const unpaidHtml = renderToStaticMarkup(
    createElement(Board, {
      ...
      listings: rankListings([
        fixtureListing({
          id: "lst_unpaid_closed_empty_plates",
          company: "Ghost",
          title: "Unpaid Staff Engineer",
          bidUsd: 50_000,
          paidUsd: 0,
  ...
  const firstHistoryPlateAt = closedEmpty.indexOf(">Backend week history<");
  const genericPlateAt = closedEmpty.search(
    /data-lane="backend"[^>]*>Backend</,
  );
  const periodAt = closedEmpty.indexOf("Closed week history 2026-W33 — read only");
  const factAt = closedEmpty.indexOf("Week 2026-W33 is read-only week history");
  ...
  assert.equal(genericPlateAt, -1);
  assert.ok(railKickerAt >= 0 && firstHistoryPlateAt > railKickerAt);
  assert.ok(periodAt > railKickerAt && factAt > periodAt);
  assert.ok(kickerAt > factAt && bodyAt > kickerAt && bidsClosedAt > bodyAt);
  ...
  assert.equal((closedEmpty.match(/week history<\/a>/g) ?? []).length, 8);
  assert.equal((closedEmpty.match(/class="wall-plate"/g) ?? []).length, 8);
  ...
  assert.doesNotMatch(closedEmpty, /empty-lane-kicker">Closed week</);
  assert.doesNotMatch(closedEmpty, /This lane was empty/);
  assert.doesNotMatch(closedEmpty, /No listings this period/);
  assert.doesNotMatch(closedEmpty, /Period 2026-W33\. Next reset/);
  assert.doesNotMatch(closedEmpty, /Next reset 2026-08-24T00:00:00\.000Z/);
  ...
  assert.doesNotMatch(closedEmpty, /This week(?:'|&#x27;|&apos;)s #1/);
  assert.doesNotMatch(closedEmpty, /Later ranks this week/);
  assert.doesNotMatch(closedEmpty, /data-week-window="rolling-7d"/);
  assert.doesNotMatch(
    closedEmpty,
    /Rolling last 7 days from paid placement\. Week 2026-W33 is an audit label/,
  ...
  assert.doesNotMatch(closedEmpty, /wall is the rolling last 7 days from paid placement/);
  assert.doesNotMatch(closedEmpty, /aria-label="Rolling last 7 days #1"/);
  assert.doesNotMatch(closedEmpty, /Later ranks in the rolling last 7 days/);
  assert.doesNotMatch(closedEmpty, /Closed week history #1/);
  assert.doesNotMatch(closedEmpty, /Later ranks in closed week history/);
  assert.doesNotMatch(closedEmpty, /This remote \(global\) wall is empty/);
  assert.doesNotMatch(closedEmpty, /Not Monday 00:00 UTC/);
  ...
  assert.doesNotMatch(
    closedEmpty,
    /data-closed-empty-plates|data-empty-function-plates-hop|data-closed-empty-plates-hop|data-empty-history-plates/,
  );
  assert.doesNotMatch(closedEmpty, /data-list-after-apply-N|data-list-after-apply-eight/);
  assert.match(closedEmpty, /wall-plate/);
  ...
  assert.doesNotMatch(closedOccupied, /aria-label="Function lanes"/);
  assert.doesNotMatch(closedOccupied, /Function lanes/);
  assert.match(closedOccupied, /data-lane="backend"[^>]*>Backend week history</);
  assert.doesNotMatch(closedOccupied, /data-lane="backend"[^>]*>Backend</);
  assert.equal((closedOccupied.match(/week history<\/a>/g) ?? []).length, 8);
  assert.match(closedOccupied, /data-listing-card/);
  assert.match(closedOccupied, />Apply</);
  assert.match(closedOccupied, /aria-label="Closed week history #1"/);
