...
test("occupied live sheet stamps Apply as the certain outbound hop", () => {
  ...
  assert.doesNotMatch(html.slice(later), /data-take-apply|data-apply-live|data-first-click="apply"/);
});
...
test("occupied #1 Apply stays the certain hop after empty-bay identity leads", () => {
  ...
  assert.ok(apply > firstClick && bid > apply);
  assert.ok(listAfter > apply && listFirst >= listAfter && bid > listFirst);
  ...
  assert.doesNotMatch(html, /data-empty-identity-first/);
  assert.doesNotMatch(html, /identity-label/);
  assert.doesNotMatch(html.slice(later), /data-apply-after-identity|data-first-click="apply"/);
});
...
test("occupied live later ranks stamp Apply so an applicant can take the hop", () => {
  ...
  assert.doesNotMatch(
    html.slice(later),
    /data-take-apply|data-apply-live|data-apply-after-identity|data-first-click="apply"|data-list-after-apply|data-list-after-apply-first/,
  );
});
...
test("occupied live #1 lists after Apply so a closer can find the write", () => {
  ...
  assert.ok(firstClick > applyLive && apply > firstClick && listAfter > apply);
  ...
  assert.doesNotMatch(
    html.slice(later),
    /data-list-after-apply|data-list-after-apply-first|after Apply|data-first-click="apply"/,
  );
});
...
test("occupied live #1 Apply wins the first click after List a role", () => {
  ...
  assert.ok(apply > firstClick && listAfter > apply);
  assert.ok(listFirst >= listAfter && hopHref > apply && hopHref < bid && listAfter < bid);
  assert.ok(afterCopy > listAfter && bid > afterCopy && later > bid);
  ...
  assert.doesNotMatch(
    html.slice(later),
    /data-first-click="apply"|data-list-after-apply|data-list-after-apply-first|after Apply/,
  );
...
test("occupied List a role stays certain after Apply wins the first click", () => {
  ...
  assert.ok(firstClick >= 0 && apply > firstClick && listAfter > apply);
